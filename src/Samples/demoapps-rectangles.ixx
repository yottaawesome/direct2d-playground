...
export module demoapps:rectangles;
import std;
import core;
...
{
    struct DemoApp final : UI::MainWindow
    {
        ~DemoApp()
        {
    ...
    private:
        ...
        void CreateDeviceResources() override
        {
            ...
            if (FAILED(hr))
                throw Error::COMError("CreateSolidColorBrush() failed [1]", hr);
            ...
            if (FAILED(hr))
                throw Error::COMError("CreateSolidColorBrush() failed [2]", hr);
        ...
        void LoadTestBitmap()
        {
            ...
            if (FAILED(hr))
                throw Error::COMError("GetFrame() failed", hr);
            ...
            if (FAILED(hr))
                throw Error::COMError("Initialize() failed", hr);
            ...
            if (FAILED(hr))
                throw Error::COMError("CreateBitmapFromWicBitmap() failed", hr);
        }
...
export namespace Rectangles
{
    ...
    try
    {
        ...
        if (!HeapSetInformation(nullptr, HeapEnableTerminationOnCorruption, nullptr, 0))
            throw Error::Win32Error("HeapSetInformation() failed", GetLastError());
